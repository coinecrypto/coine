...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            ...
            // randomly join #coine00-#coine99
            int channel_number = GetRandInt(100);
            channel_number = 0; // Litecoin: for now, just use one channel
            Send(hSocket, strprintf("JOIN #coine2%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #coine2%02d\r", channel_number).c_str());
        }
